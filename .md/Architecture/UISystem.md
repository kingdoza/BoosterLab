...
- query가 없어도 transient failure가 있으면 root를 표시한다. root enabled는 visible primary/secondary/equipment/placement/recovery 중 하나라도 실행 가능한지를 따른다.
...
- equipment row visibility/enabled/text/progress와 equipment transient failure도 C++이 갱신한다.
- placement/recovery visibility/enabled/text/progress와 intent별 transient failure도 C++이 갱신한다.
...
- placement/recovery 표현은 기존 event signature를 바꾸지 않고 별도 optional hook으로 추가한다.
...
- placement 두 개와 recovery 세 개의 이름/타입이 일치하고 LMB owner별 단일 행과 Q progress가 표시되는지 확인한다.
