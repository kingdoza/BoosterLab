...
- `FirstPersonTemplate` 런타임 모듈의 빌드 의존성 관리
...
Source/FirstPersonTemplate/
  FirstPersonTemplate.Build.cs
  FirstPersonTemplate.cpp
  FirstPersonTemplate.h
```
...
`FirstPersonTemplate.Build.cs`는 현재 범용 1인칭 조작 템플릿에 필요한 최소 런타임 의존성을 유지한다.
...
`FirstPersonTemplate.cpp`는 `IMPLEMENT_PRIMARY_GAME_MODULE`로 `FirstPersonTemplate` 모듈을 등록한다.

현재 Core System은 게임플레이 상태를 소유하지 않는다. 플레이어 입력, 카메라, 이동, sprint 상태는 Character System이 소유한다.

## Source Boundaries

- `Source/FirstPersonTemplate/Public`에는 외부 모듈 또는 Blueprint native parent가 참조할 수 있는 public header를 둔다.
- `Source/FirstPersonTemplate/Private`에는 cpp 구현과 private helper를 둔다.
...
- `CharacterSystem.md`: `Source/FirstPersonTemplate/Public/Character`, `Source/FirstPersonTemplate/Private/Character`
- `CameraSystem.md`: `Source/FirstPersonTemplate/Public/Camera`, `Source/FirstPersonTemplate/Private/Camera`
